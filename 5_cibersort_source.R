...
#main function
CIBERSORT <- function(sig_matrix, mixture_file, perm=0, QN=TRUE, absolute=FALSE, abs_method='sig.score',filename){
    if(FALSE){
			sig_matrix = file.path(lc_dir,"signature.txt")
			mixture_file = file.path(lc_dir,"mixture.txt")
    ...
    while(itor <= mixtures){
        smart_progress(ii = itor,nn = mixtures)
    ...
    #save results
    write.table(rbind(header,output),file=sprintf("CIBERSORT-Results_%s.txt",filename), sep="\t", row.names=F, col.names=F, quote=F)
